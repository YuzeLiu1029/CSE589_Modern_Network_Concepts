...
//void commandSend(int socketfdG, char ipaddressG[256], char messageG[256]);
//void commandBroadcast(char *messageG, int socketfdG);
            ...
            if(FD_ISSET(i,&readCommand_fds)){
                if(i == STDIN_FILENO) {
                    ...
                    if(strcmp(command, "AUTHOR") == 0 ){
                        ...
                        if(strcmp(signalBuf,"Y") == 0){
                            ...
                            if((recv(sockfdClient,relaybuf, sizeof(relaybuf),0)) > 0){
                                ...
                                char *relayarg_newline, relayargv_newline[20][256];
                                ...
                                for(int x = 0; x < relayargc_newline; x++){
                                    ...
                                    char *relayarg_space, relayargv_space[20][256];
                                    ...
                                    printf("%s %s %s %s %s", "From ", relaySendFromIp, " To ", relaySendToIp, relayMessage);
                                }
                            }
                        }
                        ///End of buffer data
                    ...
                    if((numbytesClient = recv(sockfdClient,bufSendFromServer, sizeof(bufSendFromServer),0)) <= 0){
                        ...
                        char *recvarg_newline, recvargv_newline[20][50];
                        ...
                        for(int x = 0; x < recvargc_newline; x++){
                            ...
                            char *recvarg_space, recvargv_space[20][50];
                            ...
                            recvMessage[strlen(recvMessage) - 1] = '\n';
                            printf("Loop1\n");
                            printf("%s %s %s %s %s", "From ", recvSendFromIp, " To ", recvSendToIp, recvMessage);
                            printf("Loop1\n");
                        }
                        ///
                        printf("Loop\n");
                       printf("%s%s\n","client : received " ,bufSendFromServer);
                    }
                }
            }
        }
...
void server(){//This Part of teh code will set up the current host as an server, waiting for connection
    ...
    char *argfromClient,argvfromClient[20][256];
    char commandfromClient[256];
    char tempcommandfromClient[256];
                ...
                if(i == STDIN_FILENO){
                        ...
                        for(int ListPrintIndex =0; ListPrintIndex < elementsInTheTable; ListPrintIndex++) {
                            if (clientTable[ListPrintIndex].status == 1) {
                                printf("%d %s %d %d %s %d\n", ListPrintIndex + 1, clientTable[ListPrintIndex].hostNameToShow,
                                       ...
                                       "logged-in",clientTable[ListPrintIndex].assignedSocket);
                            } else if (clientTable[ListPrintIndex].status == 0) {
                                printf("%d %s %d %d %s %d\n", ListPrintIndex + 1, clientTable[ListPrintIndex].hostNameToShow,
                                       clientTable[ListPrintIndex].numberofMessageSend, clientTable[ListPrintIndex].numberofMessageRec,
                                       "logged-out",clientTable[ListPrintIndex].assignedSocket);
                            }
                        }
                    ...
                    if (newfd == -1) {
                        ...
                        if(strcmp(newClient.IPToShow, "128.205.36.46") == 0){
                            ...
                            if(strlen(relayBufferStones) != 0){
                                ...
                                send(newfd,requestBuffer2,sizeof requestBuffer,0);
                            }
                        } else if(strcmp(newClient.IPToShow, "128.205.36.35") == 0){
                        ...
                        ///End Handle the buffer data of thr login client
                    }
                } else {
                    ...
                    if ((nbytes = recv(i, buf, sizeof(buf), 0)) <= 0) {
                        ...
                        if(strcmp(argvfromClient[0],"SEND") == 0){
                            ...
                            for(int qq = 0; qq < elementsInTheTable; qq++){
                                if(strcmp(clientTable[qq].IPToShow, argvfromClient[1]) == 0){ //The client doesn't exit
                                    ...
                                    if(blocklist[sendtoid][sendfromid] == 1){
                                        ...
                                        ///
                                        if(clientTable[qq].status == 0){ //The client is off line, but noe exit, need to relay message for it
                                            printf("Server : The client is offline, I will relay the message for this client.\n");
                                            ...
                                            if(strcmp(IPfromClientTable, "128.205.36.46") == 0){
                                                //stones
                                                printf("Relay message for Stones.\n");
                                ...
                                if (FD_ISSET(j, &master)) {//The client is in the live client table
                                    ...
                                    for(int tempInedxv = 0; tempInedxv < 5; tempInedxv++){
                                        if(clientTable[tempInedxv].assignedSocket == j){//The client with this socket number is logout
                                            printf("I will relay the message for you.\n");
                                            ///
                            ...
                            ////TO DO! DO NOTHING!!!! Print out the live climent list
                        }else if(strcmp(argvfromClient[0],"REFRESH") == 0){
                            ////TO DO!
                            ////The server received the referesh command and need to send the client the latested live client list
